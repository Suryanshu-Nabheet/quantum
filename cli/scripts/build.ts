/**
 * Quantum build script — bundles the TypeScript source into a single
...
const featureFlags: Record<string, boolean> = {
  ...
  VOICE_MODE: false,              // Push-to-talk STT via remote OAuth endpoint
...
result = await Bun.build({
  ...
  define: {
    ...
    // version separately in Quantum branding.
    'MACRO.VERSION': JSON.stringify('99.0.0'),
    'MACRO.DISPLAY_VERSION': JSON.stringify(version),
    'MACRO.BUILD_TIME': JSON.stringify(new Date().toISOString()),
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/SuryanshuNabheet/quantum/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/SuryanshuNabheet/quantum/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('quantum'),
...
if (!result.success) {
  ...
  console.log(`[OK] Built quantum v${version} → dist/cli.mjs`)
}
