...
import { getQuantumConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    const envVars = [{
      ...
      name: "QUANTUM_MAX_OUTPUT_TOKENS",
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getQuantumConfigHomeDir(), "agents");
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("Quantum diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
