...
import { getMemoryFiles } from '../utils/projectMemory.js';
...
import { usePromptsFromBrowserExtension } from 'src/hooks/usePromptsFromBrowserExtension.js';
...
import { useQuantumHintRecommendation } from 'src/hooks/useQuantumHintRecommendation.js';
import { PluginHintMenu } from 'src/components/QuantumHint/PluginHintMenu.js';
...
export function REPL({
  ...
  const titleDisabled = useMemo(() => isEnvTruthy(process.env.QUANTUM_DISABLE_TERMINAL_TITLE), []);
  const moreRightEnabled = useMemo(() => "external" === 'ant' && isEnvTruthy(process.env.CLAUDE_MORERIGHT), []);
  const disableVirtualScroll = useMemo(() => isEnvTruthy(process.env.QUANTUM_DISABLE_VIRTUAL_SCROLL), []);
  const disableMessageActions = true ?
    // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
    useMemo(() => isEnvTruthy(process.env.QUANTUM_DISABLE_MESSAGE_ACTIONS), []) : false;
  ...
  // from QUANTUM_NO_FLICKER=0 (which is process-lifetime) — this is
  ...
  usePromptsFromBrowserExtension(isRemoteSession ? EMPTY_MCP_CLIENTS : mcpClients, toolPermissionContext.mode);
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'Quantum';
  ...
  const sandboxAskCallback: SandboxAskCallback = useCallback(async (hostPattern: NetworkHostPattern) => {
    ...
    return new Promise(resolveShouldAllowHost => {
      ...
      // remote user (e.g. remotely) can approve it too.
      if (false) {
  ...
  const onQueryImpl = useCallback(async (messagesIncludingNewMessages: MessageType[], newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, effort?: EffortValue) => {
    ...
    // title silently fell through to the "Quantum CLI" default.
  ...
  const onSubmit = useCallback(async (input: string, helpers: PromptInputHelpers, speculationAccept?: {
    ...
    // controls treatment: "dialog" (blocking), "hint" (notification), "off".
    {
      ...
      const idleThresholdMin = Number(process.env.QUANTUM_IDLE_THRESHOLD_MINUTES ?? 75);
      const tokenThreshold = Number(process.env.QUANTUM_IDLE_TOKEN_THRESHOLD ?? 100_000);
  ...
  // for remote access via remote. No-op in external builds or when not enabled.
  const {
    sendBridgeResult
  ...
  // That write storm is the primary trigger for ~/.quantum.json corruption
  // (GH #3117).
    ...
    const timer = setTimeout((lastQueryCompletionTime, isLoading, toolJSX, focusedInputDialogRef, terminal) => {
      ...
      if (!isLoading && !toolJSX &&
        ...
        void sendNotification({
          message: 'Quantum is waiting for your input',
  ...
  useEffect(() => {
    ...
    const idleThresholdMs = Number(process.env.QUANTUM_IDLE_THRESHOLD_MINUTES ?? 75) * 60_000;
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nQuantum has been suspended. Run \`fg\` to bring Quantum back.\nNote: ctrl + z now suspends Quantum, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
