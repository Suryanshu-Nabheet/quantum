...
export function getLocalISODate(): string {
  ...
  if (process.env.QUANTUM_OVERRIDE_DATE) {
    return process.env.QUANTUM_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.QUANTUM_OVERRIDE_DATE
    ? new Date(process.env.QUANTUM_OVERRIDE_DATE)
    : new Date()
