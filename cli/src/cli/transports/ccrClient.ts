 ...
 * - Epoch management: reads worker_epoch from QUANTUM_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (QUANTUM_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * QUANTUM_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    QUANTUM_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.QUANTUM_WORKER_EPOCH
