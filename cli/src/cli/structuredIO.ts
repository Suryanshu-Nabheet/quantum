...
export class StructuredIO {
   ...
   * stale permission prompt remotely when the SDK consumer wins the race.
   */
  ...
  private async processLine(
    ...
    try {
      ...
      if (message.type === 'update_environment_variables') {
        ...
        // (QUANTUM_SESSION_ACCESS_TOKEN) which must be readable
      ...
      if (message.type === 'control_response') {
        ...
        // request, so it can cancel the stale permission prompt remotely.
        if (
