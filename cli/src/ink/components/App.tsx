...
export default class App extends PureComponent<Props, State> {
  ...
  stdinMode: 'readable' | 'data' = process.env.QUANTUM_USE_DATA_STDIN === '1' || process.env.QUANTUM_USE_READABLE_STDIN === '0' ? 'data' : 'readable';
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.QUANTUM_ACCESSIBILITY)) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Quantum CLI to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.QUANTUM_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
      ...
      // Emit resume event for Quantum CLI to handle
