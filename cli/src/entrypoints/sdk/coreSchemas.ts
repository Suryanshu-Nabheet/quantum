  ...
  z
    .object({
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.quantum/agent-memory/<agentType>/, 'project' - .claude/agent-memory/<agentType>/, 'local' - .claude/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.quantum/settings.json). " +
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for provider authentication users.'),
)
