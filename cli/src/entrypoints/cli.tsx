...
// Quantum: polyfill globalThis.File for Node < 20.
...
// Quantum: disable experimental API betas by default.
// Tool search (defer_loading), global cache scope, and context management
// require internal API support not available to external accounts → 500.
// Users can opt-in with QUANTUM_DISABLE_EXPERIMENTAL_BETAS=false.
// eslint-disable-next-line custom-rules/no-top-level-side-effects
process.env.QUANTUM_DISABLE_EXPERIMENTAL_BETAS ??= 'true'
...
if (process.env.QUANTUM_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (false && process.env.QUANTUM_ABLATION_BASELINE) {
  for (const k of ['QUANTUM_SIMPLE', 'QUANTUM_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'QUANTUM_DISABLE_AUTO_MEMORY', 'QUANTUM_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Quantum)`);
    return;
  }
  ...
  // Hydrate GitHub credentials after profile is applied so QUANTUM_USE_GITHUB from profile is available
  {
    const {
  ...
  // Session management against the ~/.quantum/sessions/ registry. Flag
  ...
  if (process.env.QUANTUM_DISABLE_EARLY_INPUT !== '1') {
    const {
