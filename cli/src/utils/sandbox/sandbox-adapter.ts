...
import { getQuantumTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve Quantum CLI-specific path patterns for sandbox-runtime.
 *
 * Quantum CLI uses special path prefixes in permission rules:
...
/**
 * Convert Quantum CLI settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  const allowWrite: string[] = ['.', getQuantumTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where Quantum CLI started)
