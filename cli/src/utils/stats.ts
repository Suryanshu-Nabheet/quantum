...
/**
 * Convert a PersistedStatsCache to QuantumStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all Quantum CLI sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateQuantumStats(): Promise<QuantumStats> {
 ...
 */
export async function aggregateQuantumStatsForRange(
  range: StatsDateRange,
): Promise<QuantumStats> {
  if (range === 'all') {
    return aggregateQuantumStats()
  }
  ...
  return processedStatsToQuantumStats(stats)
}

/**
 * Convert ProcessedStats to QuantumStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToQuantumStats(
...
function getEmptyStats(): QuantumStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
