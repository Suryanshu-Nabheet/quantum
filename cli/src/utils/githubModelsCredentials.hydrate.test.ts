...
describe('hydrateGithubModelsTokenFromSecureStorage', () => {
  const orig = {
    ...
    QUANTUM_GITHUB_TOKEN_HYDRATED:
      process.env.QUANTUM_GITHUB_TOKEN_HYDRATED,
  ...
  test('sets GITHUB_TOKEN from secure storage when USE_GITHUB and env token empty', async () => {
    ...
    expect(process.env.QUANTUM_GITHUB_TOKEN_HYDRATED).toBe('1')
  })

  test('does not override existing GITHUB_TOKEN', async () => {
    process.env.QUANTUM_USE_GITHUB = '1'
    process.env.GITHUB_TOKEN = 'already'
    delete process.env.QUANTUM_GITHUB_TOKEN_HYDRATED
    ...
    expect(process.env.QUANTUM_GITHUB_TOKEN_HYDRATED).toBeUndefined()
  })
})
