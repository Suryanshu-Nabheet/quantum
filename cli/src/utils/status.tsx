...
import { getAccountInformation, isQuantumSubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './projectMemory.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.QUANTUM_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.QUANTUM_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.QUANTUM_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.QUANTUM_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.QUANTUM_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isQuantumSubscriber()) {
