import { afterEach, describe, expect, test } from 'bun:test'
import { homedir } from 'os'
import { join } from 'path'

import { isInGlobalClaudeFolder } from '../components/permissions/FilePermissionDialog/permissionOptions.tsx'
import { optionForPermissionSaveDestination } from '../components/permissions/rules/AddPermissionRules.tsx'
import {
  getClaudeSkillScope,
  isClaudeSettingsPath,
} from './permissions/filesystem.ts'
import { getValidationTip } from './settings/validationTips.ts'

const originalConfigDir = process.env.QUANTUM_CONFIG_DIR

afterEach(() => {
  if (originalConfigDir === undefined) {
    delete process.env.QUANTUM_CONFIG_DIR
  } else {
    process.env.QUANTUM_CONFIG_DIR = originalConfigDir
  }
})

describe('Quantum settings path surfaces', () => {
  test('isClaudeSettingsPath recognizes project .quantum settings files', () => {
    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.quantum', 'settings.json'),
      ),
    ).toBe(true)

    expect(
      isClaudeSettingsPath(
        join(process.cwd(), '.quantum', 'settings.local.json'),
      ),
    ).toBe(true)
  })

  test('permission save destinations point user settings to ~/.quantum', () => {
    expect(optionForPermissionSaveDestination('userSettings')).toEqual({
      label: 'User settings',
      description: 'Saved in ~/.quantum/settings.json',
      value: 'userSettings',
    })
  })

  test('permission save destinations point project settings to .quantum', () => {
    expect(optionForPermissionSaveDestination('projectSettings')).toEqual({
      label: 'Project settings',
      description: 'Checked in at .quantum/settings.json',
      value: 'projectSettings',
    })

    expect(optionForPermissionSaveDestination('localSettings')).toEqual({
      label: 'Project settings (local)',
      description: 'Saved in .quantum/settings.local.json',
      value: 'localSettings',
    })
  })

  test('permission dialog treats ~/.quantum as the global Claude folder', () => {
    process.env.QUANTUM_CONFIG_DIR = join(homedir(), '.quantum')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), '.quantum', 'settings.json'),
      ),
    ).toBe(true)
    expect(
      isInGlobalClaudeFolder(join(homedir(), '.claude', 'settings.json')),
    ).toBe(true)
  })

  test('permission dialog does not treat arbitrary QUANTUM_CONFIG_DIR as the global Claude folder', () => {
    process.env.QUANTUM_CONFIG_DIR = join(homedir(), 'custom-quantum')

    expect(
      isInGlobalClaudeFolder(
        join(homedir(), 'custom-quantum', 'settings.json'),
      ),
    ).toBe(false)
  })

  test('global skill scope recognizes ~/.quantum and legacy ~/.quantum skills', () => {
    process.env.QUANTUM_CONFIG_DIR = join(homedir(), '.quantum')

    expect(
      getClaudeSkillScope(
        join(homedir(), '.quantum', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toEqual({
      skillName: 'demo',
      pattern: '~/.quantum/skills/demo/**',
    })

    expect(
      getClaudeSkillScope(
        join(homedir(), '.claude', 'skills', 'legacy', 'SKILL.md'),
      ),
    ).toEqual({
      skillName: 'legacy',
      pattern: '~/.quantum/skills/legacy/**',
    })
  })

  test('global skill scope does not emit fixed rules for arbitrary QUANTUM_CONFIG_DIR skills', () => {
    process.env.QUANTUM_CONFIG_DIR = join(homedir(), 'custom-quantum')

    expect(
      getClaudeSkillScope(
        join(homedir(), 'custom-quantum', 'skills', 'demo', 'SKILL.md'),
      ),
    ).toBe(null)
  })
})

describe('Quantum validation tips', () => {
  test('permissions.defaultMode invalid value keeps suggestion but no Claude docs link', () => {
    const tip = getValidationTip({
      path: 'permissions.defaultMode',
      code: 'invalid_value',
      enumValues: [
        'acceptEdits',
        'bypassPermissions',
        'default',
        'dontAsk',
        'plan',
      ],
    })

    expect(tip).toEqual({
      suggestion:
        'Valid modes: "acceptEdits" (ask before file changes), "plan" (analysis only), "bypassPermissions" (auto-accept all), or "default" (standard behavior)',
    })
  })
})
