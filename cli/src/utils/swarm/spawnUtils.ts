 ...
 * Always includes CLAUDECODE=1 and QUANTUM_EXPERIMENTAL_AGENT_TEAMS=1,
 * plus any provider/config env vars that are set in the current process.
 */
export function buildInheritedEnvVars(): string {
  const envVars = [
    'CLAUDECODE=1',
    'QUANTUM_EXPERIMENTAL_AGENT_TEAMS=1',
    // Teammates should inherit the leader-selected provider route instead of
    // replaying persisted ~/.quantum or settings.env provider defaults.
    'QUANTUM_PROVIDER_MANAGED_BY_HOST=1',
  ]
