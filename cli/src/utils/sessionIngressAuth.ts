...
function getTokenFromFileDescriptor(): string | null {
  ...
  const fdEnv = process.env.QUANTUM_WEBSOCKET_AUTH_FILE_DESCRIPTOR
  if (!fdEnv) {
  ...
  if (Number.isNaN(fd)) {
    logForDebugging(
      `QUANTUM_WEBSOCKET_AUTH_FILE_DESCRIPTOR must be a valid file descriptor number, got: ${fdEnv}`,
      { level: 'error' },
    )
 ...
 *
 * Priority order:
 *  1. Environment variable (QUANTUM_SESSION_ACCESS_TOKEN) — set at spawn time,
 ...
 *  2. File descriptor (legacy path) — QUANTUM_WEBSOCKET_AUTH_FILE_DESCRIPTOR,
...
export function getSessionIngressAuthToken(): string | null {
  ...
  const envToken = process.env.QUANTUM_SESSION_ACCESS_TOKEN
  if (envToken) {
    return envToken
  }
...
export function getSessionIngressAuthHeaders(): Record<string, string> {
  ...
  if (token.startsWith('sk-ant-sid')) {
    ...
    const orgUuid = process.env.QUANTUM_ORGANIZATION_UUID
    if (orgUuid) {
...
export function updateSessionIngressAuthToken(token: string): void {
  process.env.QUANTUM_SESSION_ACCESS_TOKEN = token
}
