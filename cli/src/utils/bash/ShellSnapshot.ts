...
/**
 * Generates Quantum CLI specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getQuantumSnapshotContent(): Promise<string> {
...
async function getSnapshotScript(
  ...
  // Generate the user content and Quantum CLI content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const quantumSnapshotContent = await getQuantumSnapshotContent()
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Quantum defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getQuantumConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.QUANTUM_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Claude home: ${getQuantumConfigHomeDir()}`)
