...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'Quantum CLI web sessions require authentication with a the consumer product account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
