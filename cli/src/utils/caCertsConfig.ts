 ...
 * user-controlled files (~/.quantum/settings.json and ~/.quantum.json),
 ...
 *
 * We read from global config (~/.quantum.json) and user settings
 * (~/.quantum/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.quantum/settings.json),
