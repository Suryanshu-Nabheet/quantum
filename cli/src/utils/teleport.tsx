...
import { checkAndRefreshOAuthTokenIfNeeded, getQuantumOAuthTokens } from './auth.js';
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getQuantumOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('Quantum CLI web sessions require authentication with a the consumer product account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run quantum --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run quantum --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run quantum --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run quantum --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote the consumer product session with error handling and UI feedback.
...
/**
 * Creates a remote the consumer product session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, QUANTUM_OAUTH_TOKEN is auto-injected from the
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        QUANTUM_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please configure GitHub for this repository' : '';
        let msg: string;
