...
test('install command displays ~/.local/bin/quantum on non-Windows', async () => {
  ...
  expect(getInstallationPath()).toBe('~/.local/bin/quantum')
})

test('install command displays quantum.exe path on Windows', async () => {
  mock.module('../utils/env.js', () => ({
    env: { platform: 'win32' },
  }))
  ...
  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'quantum.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both quantum and legacy claude local install dirs', async () => {
  ...
  expect(removedPaths).toContain(join(homedir(), '.quantum', 'local'))
