 ...
 * env vars that the remote's ~/.quantum settings.env MUST NOT clobber (see
 ...
 * QUANTUM_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.quantum/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.QUANTUM_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.quantum/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.quantum.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.quantum/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads QUANTUM_USE_BEDROCK,
  ...
  // when QUANTUM_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * QUANTUM_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
