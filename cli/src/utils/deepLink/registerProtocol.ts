...
const APP_NAME = 'Quantum URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'Quantum URL Handler.app'
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Quantum Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for Quantum
 ...
 * a cached flag in ~/.quantum.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.quantum (per-machine, not synced) rather than ~/.quantum.json (can sync).
