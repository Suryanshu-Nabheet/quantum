 ...
 *
 * Priority order:
 *   QUANTUM_NO_FLICKER=0  → always off
 *   QUANTUM_NO_FLICKER=1  → always on (overrides tmux -CC guard too)
...
export function isFullscreenEnvEnabled(): boolean {
  ...
  if (isEnvDefinedFalsy(process.env.QUANTUM_NO_FLICKER)) return false
  // Explicit env opt-in overrides everything including tmux -CC.
  if (isEnvTruthy(process.env.QUANTUM_NO_FLICKER)) return true
  ...
  if (isTmuxControlMode()) {
    if (!loggedTmuxCcDisable) {
      ...
      logForDebugging(
        'fullscreen disabled: tmux -CC (iTerm2 integration mode) detected · set QUANTUM_NO_FLICKER=1 to override',
      )
    }
    return false
  }
 ...
 * Set QUANTUM_DISABLE_MOUSE=1 to keep alt-screen + virtualized scroll
 ...
 *
 * Compare with QUANTUM_NO_FLICKER=0 which is all-or-nothing — it also
 * disables alt-screen and virtualized scrollback.
 */
export function isMouseTrackingEnabled(): boolean {
  return !isEnvTruthy(process.env.QUANTUM_DISABLE_MOUSE)
}

/**
 * Whether mouse click handling is disabled (clicks/drags ignored, wheel still
 * works). Set QUANTUM_DISABLE_MOUSE_CLICKS=1 to prevent accidental clicks
 * from triggering cursor positioning, text selection, or message expansion.
 *
 * Fullscreen-specific — only reachable when QUANTUM_NO_FLICKER is active.
 */
export function isMouseClicksDisabled(): boolean {
  return isEnvTruthy(process.env.QUANTUM_DISABLE_MOUSE_CLICKS)
}
