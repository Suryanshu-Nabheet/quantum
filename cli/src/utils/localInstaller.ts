...
// hfi.tsx get a chance to set QUANTUM_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getQuantumConfigHomeDir(), 'local')
}
...
export function getCandidateLocalInstallDirs(options?: {
  ...
  const configHomeDir = options?.configHomeDir ?? getQuantumConfigHomeDir()
  return Array.from(
...
function getCandidateLocalBinaryPaths(localInstallDir: string): string[] {
  return [
    join(localInstallDir, 'node_modules', '.bin', 'quantum'),
...
export function isManagedLocalInstallationPath(execPath: string): boolean {
  ...
  return (
    normalizedExecPath.includes('/.quantum/local/node_modules/') ||
    normalizedExecPath.includes('/.claude/local/node_modules/')
  )
}

export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'quantum')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'quantum-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/quantum" "$@"`,
      0o755,
    )
    if (created) {
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install Quantum CLI package: ${result.stderr}`,
      )
      logError(error)
