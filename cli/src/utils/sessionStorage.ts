...
import { getQuantumConfigHomeDir, getProjectsDir, isEnvTruthy } from './envUtils.js'
...
function getEntrypoint(): string | undefined {
  return process.env.QUANTUM_ENTRYPOINT
}
 ...
 * This ensures tests with different QUANTUM_CONFIG_DIR values
...
class Project {
   ...
   * QUANTUM_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.QUANTUM_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.QUANTUM_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // QUANTUM_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.QUANTUM_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.QUANTUM_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
