...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'quantum', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'quantum')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'quantum', { status: 'alpha' })
    await addGlobalEntity('tool', 'quantum', { status: 'beta', version: '0.6.0' })
