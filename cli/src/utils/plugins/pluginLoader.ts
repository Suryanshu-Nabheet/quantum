 ...
 *
 * This module is responsible for discovering, loading, and validating Quantum CLI plugins
 ...
 * Format: ~/.quantum/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.quantum/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.QUANTUM_REMOTE)
 ...
 * QUANTUM_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
 ...
 *
 * QUANTUM_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getQuantumMcpConfigs()/getInitialSettings().agent run
