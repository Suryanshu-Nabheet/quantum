/**
 * Marketplace manager for Quantum CLI plugins
 *
 ...
 *
 * Reads the configuration file at ~/.quantum/plugins/known_marketplaces.json
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Quantum CLI bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.quantum/plugins/known_marketplaces.json,
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via QUANTUM_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.QUANTUM_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via QUANTUM_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via QUANTUM_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set QUANTUM_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Quantum-Plugin-Manager',
  }
 ...
 * The configuration is saved to ~/.quantum/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: quantum plugin disable <plugin>@${name}`,
    )
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `an older Quantum version. Run 'quantum plugin marketplace remove ${name}' and ` +
  ...
  try {
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: quantum plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.QUANTUM_REMOTE)) {
      ...
      try {
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: quantum plugin marketplace remove "${name}"`,
        )
      }
