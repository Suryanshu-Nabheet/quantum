 ...
 * When QUANTUM_PLUGIN_USE_ZIP_CACHE is enabled and QUANTUM_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.QUANTUM_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires QUANTUM_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.QUANTUM_PLUGIN_CACHE_DIR
