...
export async function validatePluginManifest(
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Quantum ` +
  ...
  if (result.success) {
    ...
    // but the the consumer product marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Quantum accepts ` +
          `it, but the the consumer product marketplace sync requires kebab-case ` +
