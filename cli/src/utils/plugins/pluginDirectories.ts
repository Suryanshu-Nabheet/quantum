 ...
 * - CLI flag: --cowork
 * - Environment variable: QUANTUM_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via QUANTUM_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable QUANTUM_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.QUANTUM_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. QUANTUM_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.quantum/plugins or ~/.quantum/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when QUANTUM_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.quantum/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.QUANTUM_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getQuantumConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point QUANTUM_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.QUANTUM_PLUGIN_SEED_DIR
  if (!raw) return []
