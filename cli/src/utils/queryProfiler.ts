 ...
 * pipeline from user input to first token arrival. Enable by setting QUANTUM_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.QUANTUM_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set QUANTUM_PROFILE_QUERY=1)'
  }
