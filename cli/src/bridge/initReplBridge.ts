...
export type InitBridgeOptions = {
   ...
   * stream). Used by CCR mirror mode — local sessions visible remotely
...
export async function initReplBridge(
  ...
  // 2. Check OAuth — must be signed in with the remote client. Runs before the
  ...
  if (!getBridgeTokenOverride()) {
    ...
    if (
      ...
      getQuantumOAuthTokens()?.expiresAt === cfg.bridgeOauthDeadExpiresAt
    ) {
      logForDebugging(
  ...
  // auto-started sessions distinguishable in the session list before the
  // first prompt.
  ...
  // The env-based path below can ALSO use CCR v2 via QUANTUM_USE_CCR_V2.
  ...
  if (isEnvLessBridgeEnabled() && !perpetual) {
    ...
    if (versionError) {
      ...
      onStateChange?.('failed', 'run `quantum update` to upgrade')
      return null
    }
    logForDebugging(
...
function deriveTitle(raw: string): string | undefined {
  ...
  // Collapse newlines/tabs — titles are single-line in the session list.
