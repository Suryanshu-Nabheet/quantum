 ...
 * (oauth-authed, same store), write to ~/.quantum/uploads/{sessionId}/, and
...
function uploadsDir(): string {
  return join(getQuantumConfigHomeDir(), 'uploads', getSessionId())
}
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // QUANTUM_CUSTOM_OAUTH_URL — keep it inside the try so a bad
