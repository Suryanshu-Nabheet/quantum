...
// Only QUANTUM_OAUTH_TOKEN was stripped. Fix: explicit allowlist.
...
test('buildChildEnv does not leak ANTHROPIC_API_KEY to child', () => {
  const parentEnv = {
    ...
    QUANTUM_SESSION_ACCESS_TOKEN: 'will-be-overwritten',
  }
...
test('buildChildEnv sets QUANTUM_SESSION_ACCESS_TOKEN from opts', () => {
  const env = buildChildEnv({ PATH: '/usr/bin' }, { ...baseOpts, accessToken: 'my-token' })
  expect(env.QUANTUM_SESSION_ACCESS_TOKEN).toBe('my-token')
})

test('buildChildEnv sets QUANTUM_ENVIRONMENT_KIND to bridge', () => {
  const env = buildChildEnv({ PATH: '/usr/bin' }, baseOpts)
  expect(env.QUANTUM_ENVIRONMENT_KIND).toBe('bridge')
})

test('buildChildEnv does not pass QUANTUM_OAUTH_TOKEN to child', () => {
  const parentEnv = {
    PATH: '/usr/bin',
    QUANTUM_OAUTH_TOKEN: 'oauth-token-to-strip',
  }
  const env = buildChildEnv(parentEnv, baseOpts)
  expect(env.QUANTUM_OAUTH_TOKEN).toBeUndefined()
})
...
test('buildChildEnv sets CCR v2 vars when useCcrV2 is true', () => {
  ...
  expect(env.QUANTUM_USE_CCR_V2).toBe('1')
  expect(env.QUANTUM_WORKER_EPOCH).toBe('42')
})
