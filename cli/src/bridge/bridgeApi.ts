        ...
        (token: string) =>
          axios.post<{
            ...
            `${deps.baseUrl}/v1/environments/bridge`,
            {
              ...
              // Advertise session capacity so Quantum can show
              ...
              // worker_type lets remote filter environments by origin
    ...
    case 403:
      throw new BridgeFatalError(
        isExpiredErrorType(errorType)
          ? 'Remote Control session has expired. Please restart with `quantum remote-control` or /remote-control.'
