 ...
 * `QUANTUM_DISABLE_CRON=1` to turn it off locally.
 *
 ...
 * chance to populate.
 *
 * `QUANTUM_DISABLE_CRON` is a local override that wins over GB.
 */
export function isKairosCronEnabled(): boolean {
  if (isEnvTruthy(process.env.QUANTUM_DISABLE_CRON)) return false

  // Quantum open builds do not rely on Anthropic's internal runtime gates.
 ...
 * durable cron. Does NOT consult QUANTUM_DISABLE_CRON (that kills the whole
...
By default (durable: false) the job lives only in this Quantum session — nothing is written to disk, and the job is gone when Quantum exits. Pass durable: true to write to .claude/scheduled_tasks.json so the job survives restarts. Only use durable: true when the user explicitly asks for the task to persist ("keep doing this every day", "set this up permanently"). Most "remind me in 5 minutes" / "check back in an hour" requests should stay session-only.`
    : `## Session-only

Jobs live only in this Quantum session — nothing is written to disk, and the job is gone when Quantum exits.`
