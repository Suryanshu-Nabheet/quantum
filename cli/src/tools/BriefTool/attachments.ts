...
export async function resolveAttachments(
  ...
  if (false) {
    ...
    // REPL does, at main.tsx init). QUANTUM_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes QUANTUM_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.QUANTUM_BRIEF_UPLOAD)
