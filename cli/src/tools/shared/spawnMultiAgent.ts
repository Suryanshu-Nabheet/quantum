...
async function handleSpawnSplitPane(
  ...
  // Build the command to spawn Quantum CLI with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces QUANTUM_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, QUANTUM_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Quantum CLI instance.
