...
//
// Set `QUANTUM_LOCAL_FAST_PATH=1` to force it on, `=0` to force off, or
...
const LOCAL_FAST_PATH_ENV = 'QUANTUM_LOCAL_FAST_PATH'
...
export function resolveProviderRequest(options?: {
  ...
  const isGithubMode = isEnvTruthy(process.env.QUANTUM_USE_GITHUB)
  const isMistralMode = isEnvTruthy(process.env.QUANTUM_USE_MISTRAL)
  const isGeminiMode = isEnvTruthy(process.env.QUANTUM_USE_GEMINI)
...
export function getAdditionalModelOptionsCacheScope(): string | null {
  if (!isEnvTruthy(process.env.QUANTUM_USE_OPENAI)) {
    if (!isEnvTruthy(process.env.QUANTUM_USE_GEMINI) &&
