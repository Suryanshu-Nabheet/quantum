...
import { getQuantumTempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getQuantumTempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
  ...
  effortValue: string
  /** Hash of getExtraBodyParams() — catches QUANTUM_EXTRA_BODY and
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getQuantumTempDir(), { recursive: true })
