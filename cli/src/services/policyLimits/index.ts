 ...
 * - OAuth users (the consumer product): Only Team and Enterprise/C4E subscribers are eligible
...
function getCachePath(): string {
  return join(getQuantumConfigHomeDir(), CACHE_FILENAME)
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have the consumer product tokens
  const tokens = getQuantumOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have the consumer product inference scope
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for the consumer product users)
