...
/**
 * CCR proxy URL path markers. In remote sessions, disabled connectors arrive
...
/**
 * Filter disabled connectors, dropping any whose signature matches an enabled
 ...
 *
 * Connector keys are `connector <DisplayName>` so they never key-collide with
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing disabled connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
 ...
 * in getQuantumMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getQuantumMcpConfigs().
...
/**
 * Get Quantum CLI MCP configurations (excludes remote servers from the
 ...
 * disabled connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Quantum CLI server configurations with appropriate scopes
 */
export async function getQuantumMcpConfigs(
...
/**
 * Get all MCP configurations across all scopes, including remote servers.
 * This may be slow due to network calls - use getQuantumMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load remote servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getQuantumMcpConfigs()
  }

  // Kick off the remote fetch before getQuantumMcpConfigs so it overlaps
  ...
  const { servers: claudeCodeServers, errors } = await getQuantumMcpConfigs(
    {},
    claudeaiPromise,
  )
  ...
  // Suppress disabled connectors that duplicate an enabled manual server.
  // Keys never collide (`slack` vs `connector Slack`) so the merge below
  ...
  // Merge with the remote client having lowest precedence
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://github.com/Suryanshu-Nabheet/Quantum/blob/main/cli/README.md`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
