 ...
 *
 * Per RFC 7009, public clients (like Quantum CLI) should authenticate by including
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'quantum mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but QUANTUM_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set QUANTUM_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
    ...
    const authorizationCode = await new Promise<string>((resolve, reject) => {
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(
            `<h1>Authentication Successful</h1><p>You can close this window. Return to Quantum.</p>`,
          )
          cleanup()
          resolveOnce(result.code)
        }
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Quantum CLI (${this.serverName})`,
