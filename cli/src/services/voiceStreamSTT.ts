...
// OAuth credentials as Quantum CLI.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Quantum CLI — available when the
  // user is authenticated with Anthropic (the consumer product subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // visibility.external: true). We target that host instead of the remote host
  // because the remote CF zone uses TLS fingerprinting and challenges
  ...
  // Desktop dictation still uses remote (Swift URLSession has a
