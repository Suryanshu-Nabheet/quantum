 ...
 *   1. QUANTUM_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. QUANTUM_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no QUANTUM_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.QUANTUM_DISABLE_AUTO_MEMORY
  ...
  if (
    ...
    !process.env.QUANTUM_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. QUANTUM_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.quantum (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.QUANTUM_REMOTE_MEMORY_DIR) {
    return process.env.QUANTUM_REMOTE_MEMORY_DIR
  }
  return getQuantumConfigHomeDir()
}
 ...
 * env vars / settings.json / QUANTUM_CONFIG_DIR are session-stable in
