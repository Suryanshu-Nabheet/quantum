...
import { getQuantumConfigHomeDir, isEnvTruthy } from './utils/envUtils.js'
...
/**
 * Quantum CLI parses history for pasted content references to match back to
...
async function* makeLogEntryReader(): AsyncGenerator<LogEntry> {
  ...
  const historyPath = join(getQuantumConfigHomeDir(), 'history.jsonl')
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Quantum CLI's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.QUANTUM_SKIP_PROMPT_HISTORY)) {
    return
  }
