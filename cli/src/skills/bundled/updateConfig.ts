...
|------|-------|-----|---------|
| \`~/.quantum/settings.json\` | Global | N/A | Personal preferences for all projects |
...
Hooks run commands at specific points in Quantum CLI's lifecycle.
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "jq -r '.tool_input.command' >> ~/.quantum/bash-log.txt"
      }]
    }]
  }
}
...
Modify Quantum CLI configuration by updating settings.json files.
...
1. **Check the settings file** - Read ~/.quantum/settings.json or .claude/settings.json
2. **Verify JSON syntax** - Invalid JSON silently fails
3. **Check the matcher** - Does it match the tool name? (e.g., "Bash", "Write", "Edit")
4. **Check hook type** - Is it "command", "prompt", or "agent"?
5. **Test the command** - Run the hook command manually to see if it works
6. **Use --debug** - Run \`quantum --debug\` to see hook execution logs
`
