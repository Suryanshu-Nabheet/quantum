...
export function SelectEventMode(t0) {
  ...
  if ($[2] !== restrictedByPolicy) {
    t2 = restrictedByPolicy && <Box flexDirection="column"><Text color="suggestion">{figures.info} Hooks Restricted by Policy</Text><Text dimColor={true}>Only hooks from managed settings can run. User-defined hooks from ~/.quantum/settings.json, .claude/settings.json, and .claude/settings.local.json are blocked.</Text></Box>;
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column"><Text dimColor={true}>{figures.info} This menu is read-only. To add or modify hooks, edit settings.json directly or ask {PRODUCT_DISPLAY_NAME}.{" "}<Link url="https://github.com/Suryanshu-Nabheet/Quantum/blob/main/cli/README.md">Learn more</Link></Text></Box>;
    $[4] = t3;
  } else {
    t3 = $[4];
  }
