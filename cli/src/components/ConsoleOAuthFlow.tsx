  ...
  const startOAuth = useCallback(async () => {
    try {
      ...
      if (mode === 'setup-token') {
        ...
        // Don't save to keychain - the token is displayed for manual use with QUANTUM_OAUTH_TOKEN
        setOAuthStatus({
          state: 'success',
        ...
        void sendNotification({
          message: 'Quantum login successful',
...
function OAuthStatusMessage({
  ...
  switch (oauthStatus.state) {
    case 'idle': {
      const promptText =
        startingMessage ||
        'Quantum can be used with your Claude subscription or billed based on API usage through your Console account.'
    ...
    case 'creating_api_key':
      return (
        <Box flexDirection="column" gap={1}>
          <Box>
            <Spinner />
            <Text>Creating API key for Quantum…</Text>
          </Box>
        </Box>
      )
