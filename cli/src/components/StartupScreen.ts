/**
 * Quantum startup screen — filled-block text logo with sunset gradient.
 * Called once at CLI startup before the Ink UI renders.
 *
 * Addresses: https://github.com/SuryanshuNabheet/quantum/issues/55
 */
...
export function detectProvider(modelOverride?: string): { name: string; model: string; baseUrl: string; isLocal: boolean } {
  const useGemini = process.env.QUANTUM_USE_GEMINI === '1' || process.env.QUANTUM_USE_GEMINI === 'true'
  const useGithub = process.env.QUANTUM_USE_GITHUB === '1' || process.env.QUANTUM_USE_GITHUB === 'true'
  const useOpenAI = process.env.QUANTUM_USE_OPENAI === '1' || process.env.QUANTUM_USE_OPENAI === 'true'
  const useMistral = process.env.QUANTUM_USE_MISTRAL === '1' || process.env.QUANTUM_USE_MISTRAL === 'true'
...
export function printStartupScreen(modelOverride?: string): void {
  ...
  out.push(`${ansiRgb(...BORDER)}\u255a${'\u2550'.repeat(W - 2)}\u255d${RESET}`)
  out.push(`  ${DIM}${ansiRgb(...DIMCOL)}quantum ${RESET}${ansiRgb(...ACCENT)}v${MACRO.DISPLAY_VERSION ?? MACRO.VERSION}${RESET}`)
  out.push('')
