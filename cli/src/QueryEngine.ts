  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.QUANTUM_EAGER_FLUSH) ||
          isEnvTruthy(process.env.QUANTUM_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via QUANTUM_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or QUANTUM_PLUGIN_SEED_DIR before this runs;
