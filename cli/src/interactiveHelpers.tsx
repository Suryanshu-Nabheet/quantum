...
import { getExternalClaudeMdIncludes, getMemoryFiles, shouldShowClaudeMdExternalIncludesWarning } from './utils/projectMemory.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (false || false) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      if (!isChannelsEnabled() || !getQuantumOAuthTokens()?.accessToken) {
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.QUANTUM_FRAME_TIMING_LOG;
  return {
