...
describe('onboarding auth precedence cleanup', () => {
  test('clears preexisting OpenAI auth when switching to GitHub', () => {
    const env: NodeJS.ProcessEnv = {
      ...
      QUANTUM_PROVIDER_PROFILE_ENV_APPLIED_ID: 'profile_old',
    }

    applyGithubOnboardingProcessEnv('github:copilot', env)

    expect(env.QUANTUM_USE_GITHUB).toBe('1')
    ...
    expect(env.QUANTUM_USE_OPENAI).toBeUndefined()
    expect(env.QUANTUM_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()
    expect(env.QUANTUM_PROVIDER_PROFILE_ENV_APPLIED_ID).toBeUndefined()

    const settingsEnv = buildGithubOnboardingSettingsEnv('github:copilot')
    expect(settingsEnv.QUANTUM_USE_GITHUB).toBe('1')
