...
export function buildGithubOnboardingSettingsEnv(
  ...
  return {
    ...
    QUANTUM_USE_GEMINI: undefined,
    QUANTUM_USE_BEDROCK: undefined,
    QUANTUM_USE_VERTEX: undefined,
    QUANTUM_USE_FOUNDRY: undefined,
  }
}
...
export function applyGithubOnboardingProcessEnv(
  ...
  delete env.QUANTUM_USE_OPENAI
  delete env.QUANTUM_USE_GEMINI
  delete env.QUANTUM_USE_BEDROCK
  delete env.QUANTUM_USE_VERTEX
  delete env.QUANTUM_USE_FOUNDRY
  delete env.QUANTUM_PROVIDER_PROFILE_ENV_APPLIED
  delete env.QUANTUM_PROVIDER_PROFILE_ENV_APPLIED_ID
}
...
function OnboardGithub(props: {
  ...
  const finalize = useCallback(
    async (
      ...
      if (!activated.ok) {
        setErrorMsg(
          `Token saved, but settings were not updated: ${activated.detail ?? 'unknown error'}. ` +
            `Add env QUANTUM_USE_GITHUB=1 and OPENAI_MODEL to ~/.quantum/settings.json manually.`,
        )
        setStep('error')
        return
      }
      ...
      onChangeAPIKey()
      onDone(
        'GitHub Copilot onboard complete. Copilot token and OAuth token stored in secure storage (Windows/Linux: ~/.quantum/.credentials.json, macOS: Keychain fallback to ~/.quantum/.credentials.json); user settings updated. Restart if the model does not switch.',
        { display: 'user' },
      )
    },
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text dimColor>
        ...
        and enables QUANTUM_USE_GITHUB in your user settings - no export
    ...
    if (!activated.ok) {
      onDone(
        `GitHub token detected, but settings activation failed: ${activated.detail ?? 'unknown error'}. ` +
          'Set QUANTUM_USE_GITHUB=1 and OPENAI_MODEL=github:copilot in user settings manually.',
        { display: 'system' },
      )
      return null
    }
