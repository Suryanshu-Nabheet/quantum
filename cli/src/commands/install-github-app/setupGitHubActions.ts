...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: A Quantum workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· For manual setup → Visit: https://github.com/Suryanshu-Nabheet/Quantum'
