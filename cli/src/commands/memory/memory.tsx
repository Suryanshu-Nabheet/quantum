...
import { clearMemoryFileCaches, getMemoryFiles } from '../../utils/projectMemory.js';
...
function MemoryCommand({
  ...
  const handleSelectMemoryFile = async (memoryPath: string) => {
    try {
      ...
      if (memoryPath.includes(getQuantumConfigHomeDir())) {
        await mkdir(getQuantumConfigHomeDir(), {
          recursive: true
        });
      }
  ...
  return <Dialog title="Memory" onCancel={handleCancel} color="remember">
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <Text dimColor>
            Learn more: <Link url="https://github.com/Suryanshu-Nabheet/Quantum/blob/main/cli/README.md" />
          </Text>
        </Box>
      </Box>
    </Dialog>;
